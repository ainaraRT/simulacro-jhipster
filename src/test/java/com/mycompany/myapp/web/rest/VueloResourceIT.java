...
class VueloResourceIT {
    ...
    @Test
    @Transactional
    void getAllVuelosByTripulantesIsEqualToSomething() throws Exception {
        // Initialize the database
        vueloRepository.saveAndFlush(vuelo);
        Tripulante tripulantes;
        if (TestUtil.findAll(em, Tripulante.class).isEmpty()) {
            tripulantes = TripulanteResourceIT.createEntity(em);
            em.persist(tripulantes);
            em.flush();
        } else {
            tripulantes = TestUtil.findAll(em, Tripulante.class).get(0);
        }
        em.persist(tripulantes);
        em.flush();
        vuelo.addTripulantes(tripulantes);
        vueloRepository.saveAndFlush(vuelo);
        Long tripulantesId = tripulantes.getId();

        // Get all the vueloList where tripulantes equals to tripulantesId
        defaultVueloShouldBeFound("tripulantesId.equals=" + tripulantesId);

        // Get all the vueloList where tripulantes equals to (tripulantesId + 1)
        defaultVueloShouldNotBeFound("tripulantesId.equals=" + (tripulantesId + 1));
    }
